...
class gfx9:
    def __init__(self, numXCDs=8, chunkSize=1, numCUsPerXCD=38, L2BytesPerXCD=2097152, MALLBytes=268435456):
...
class WorkGroup:
    def __init__(self, m, n, new_m, new_n, xcd, cu, color, M=8, N=16, width=0.1, height=0.1):
        self.m = m
        self.n = n
        self.new_m = new_m
        self.new_n = new_n
...
class Problem:
    ...
    def __init__(self, M, N, K, WGM, GPU, MT0=128, MT1=128, DU=64, workGroupsPerCU=1, width=0.1, height=0.1, elemSize=2, customWGM=True):
        self.M = M
        self.N = N
        self.K = K
                    ...
                    if ATile_str not in L2[wg.xcd].keys():
                        ...
                        if ATile_str not in MALL.keys():
                            ...
                            if MALLUsage + self.ATileBytes >= self.GPU.MALLBytes:
                                ...
                                lru_entry = min(AMLL, key=MALL.get)
        ...
        if partialWave:
            for kSlice in range(self.KOverDU):
                for wgNumWithinPartialWave in range(workGroupsPerPartialWave):
                    ...
                    if ATile_str not in L2[wg.xcd]:
                        if L2Usage[wg.xcd] + self.ATileBytes >= self.GPU.L2BytesPerXCD:
                            L2[wg.xcd].pop(0)
                            L2Usage[wg.xcd] -= self.ATileBytes
                        L2[wg.xcd].update({ATile_str: clk})
                        L2Usage[wg.xcd] += self.ATileBytes
                        if ATile_str not in MALL:
                            if MALLUsage + self.ATileBytes >= self.GPU.MALLBytes:
                                MALL.pop(0)
                                MALLUsage -= self.ATileBytes
                            MALL.update({ATile_str: clk})
